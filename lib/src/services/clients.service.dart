import 'package:http/http.dart' as http;
import 'package:odee_front/src/cross/globals.dart';
import 'package:odee_front/src/models/models.dart';
import '../abstract_class/abstract_class.dart';

class ClientsService extends MaintainerService {
  /* Singleton */
  static final ClientsService _clientsService = ClientsService._();
  factory ClientsService() {
    return _clientsService;
  }
  ClientsService._();

  @override
  Future<ResponseModel<List<ClientsModel>>> read() async {
    final resp = await http.get(Globals.getUri("/Clients"));
    final decodeResp = ResponseModel<List<dynamic>>.fromJson((resp.body));
    final payload =
        decodeResp.payload.map((e) => ClientsModel.fromMap(e)).toList();
    final respFinal =
        ResponseModel(message: decodeResp.message, payload: payload);
    respFinal.code = resp.statusCode;
    return respFinal;
  }
}
